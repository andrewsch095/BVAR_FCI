...
cols = [
    "Date", "GPR", "Brent", "for_invest_OFZ", "GDP_(%)_m/m_real_2021",
    "unempl", "net_exp", "internal_debt", "real_eff_exchange_rate_index_m/m",
    "proces_ind", "Inflation_m/m_without_seas", "Interest_rate_(%)",
    "Fed_Bonds_10", "spread_diff", "IMOEX", "M2X", "exp_inf_firms_seas"
]
df = data[cols].copy()

# GDP: x/100 - 1
gdp_col = df["GDP_(%)_m/m_real_2021"].values.copy()
for i in range(1, len(gdp_col)):
    df.loc[df.index[i], "GDP_(%)_m/m_real_2021"] = gdp_col[i] / 100 - 1

# Inflation: x/100
vals = df["Inflation_m/m_without_seas"].values.copy()
for i in range(1, len(vals)):
    df.loc[df.index[i], "Inflation_m/m_without_seas"] = vals[i] / 100

# Log-diff
for col in ["IMOEX", "GPR", "Brent", "M2X", "internal_debt", "net_exp"]:
    vals = df[col].values.copy()
    for i in range(1, len(vals)):
        df.loc[df.index[i], col] = np.log(vals[i]) - np.log(vals[i - 1])

# x/100
for col in ["real_eff_exchange_rate_index_m/m", "proces_ind"]:
    vals = df[col].values.copy()
    for i in range(1, len(vals)):
        df.loc[df.index[i], col] = vals[i] / 100

# First diff of rates (/100)
for col in ["Interest_rate_(%)", "Fed_Bonds_10", "exp_inf_firms_seas", "for_invest_OFZ"]:
    vals = df[col].values.copy()
    for i in range(1, len(vals)):
        df.loc[df.index[i], col] = (vals[i] / 100) - (vals[i - 1] / 100)

# spread_diff: /10000 first diff
vals = df["spread_diff"].values.copy()
for i in range(1, len(vals)):
    df.loc[df.index[i], "spread_diff"] = (vals[i] / 10000) - (vals[i - 1] / 10000)

# Drop first row (used as base for diffs)
...
def _riwish(nu: int, Psi: np.ndarray) -> np.ndarray:
    """Sample from Inverse-Wishart(nu, Psi) via rWishart(nu, Psi^-1)^-1."""
    n = Psi.shape[0]
    Psi_inv = np.linalg.inv(Psi + np.eye(n) * 1e-10)
    L = np.linalg.cholesky(Psi_inv + np.eye(n) * 1e-10)
    Z = np.zeros((n, n))
    for i in range(n):
        Z[i, i] = np.sqrt(np.random.chisquare(nu - i))
        if i > 0:
...
def gibbs_bvar(Y: np.ndarray, X: np.ndarray, prior: dict,
    ...
    """Gibbs sampler for BVAR with Minnesota prior."""
    T_obs, N = Y.shape
    K = X.shape[1]
...
n_draws  = 5000
n_burn   = 1000
gdp_idx  = var_names.index("GDP_(%)_m/m_real_2021")
...
B_mean     = bvar_results["B_draws"].mean(axis=2)
Sigma_mean = bvar_results["Sigma_draws"].mean(axis=2)

# ================================================================
# ДИАГНОСТИКА 1: Geweke z-scores + ESS
# ================================================================
...
def effective_size(chain: np.ndarray, max_lag: int = 400) -> float:
    n = len(chain)
    c = chain - chain.mean()
    acf_vals = np.array([np.dot(c[:n - k], c[k:]) / np.dot(c, c)
...
for i in range(N_vars):
    chain = bvar_results["B_draws"][i, i, :]
    gz    = geweke_z(chain)
...
for i in range(N_vars):
    chain = bvar_results["Sigma_draws"][i, i, :]
    gz    = geweke_z(chain)
    ess   = effective_size(chain)
    all_z.append(gz)
...
chain = bvar_results["B_draws"][gdp_idx, gdp_idx, :]
axes[0, 0].plot(chain, color="blue", lw=0.8)
axes[0, 0].axhline(chain.mean(), color="red", lw=2)
axes[0, 0].set_title("Trace: GDP own lag-1"); axes[0, 0].set_xlabel("Draw")

chain2 = bvar_results["Sigma_draws"][gdp_idx, gdp_idx, :]
axes[0, 1].plot(chain2, color="darkgreen", lw=0.8)
axes[0, 1].axhline(chain2.mean(), color="red", lw=2)
axes[0, 1].set_title("Trace: Sigma[GDP,GDP]"); axes[0, 1].set_xlabel("Draw")

n_lags_acf = 40
acf_vals = np.array([np.corrcoef(chain[:-k], chain[k:])[0, 1] for k in range(1, n_lags_acf + 1)])
axes[1, 0].bar(range(1, n_lags_acf + 1), acf_vals)
axes[1, 0].axhline(0, color="black"); axes[1, 0].set_title("ACF: GDP own lag-1")

axes[1, 1].hist(chain, bins=50, color="lightblue", edgecolor="gray")
...
plt.tight_layout()
plt.savefig("trace_plots_diagnostics.pdf")
plt.show()
print("Saved: trace_plots_diagnostics.pdf")
...
def compute_irf_bvar(B_draws: np.ndarray, Sigma_draws: np.ndarray,
    ...
    for d in tqdm(range(n_dr)):
        ...
        try:
            P = np.linalg.cholesky(Sigma_d + np.eye(N) * 1e-10).T  # upper
            P = P.T                                                  # lower
        except np.linalg.LinAlgError:
            P = np.diag(np.sqrt(np.diag(Sigma_d)))
...
n_periods   = 12
irf_results = compute_irf_bvar(bvar_results["B_draws"], bvar_results["Sigma_draws"], n_periods)


def plot_var_irf(irf_results: dict, var_names: list, var_norm_names: dict,
                 target_idx: int, n_periods: int = 12):
    ...
    plt.tight_layout()
    plt.savefig(f"irf_{target_name.replace(' ','_')}.png", dpi=120)
    plt.show()


inf_idx = var_names.index("Inflation_m/m_without_seas")
m2x_idx = var_names.index("M2X")

print("\n=== IRFs ===")
plot_var_irf(irf_results, var_names, var_norm_names, gdp_idx)
plot_var_irf(irf_results, var_names, var_norm_names, inf_idx)
plot_var_irf(irf_results, var_names, var_norm_names, m2x_idx)
...
for d_i, d in enumerate(draw_idx):
    mean_mat = X @ bvar_results["B_draws"][:, :, d]
    S = bvar_results["Sigma_draws"][:, :, d]
    try:
        L = np.linalg.cholesky(S + np.eye(N_vars) * 1e-10)
    except np.linalg.LinAlgError:
        L = np.diag(np.sqrt(np.abs(np.diag(S))))
    Y_sim[:, :, d_i] = mean_mat + np.random.randn(Y.shape[0], N_vars) @ L.T

hdr = f"{'Variable':<20s} {'Act.Mean':>10s} {'Sim.Mean':>10s} {'Act.SD':>10s} {'Sim.SD':>10s} {'p-value':>10s}"
print(hdr)
print("-" * 82)
for i, name in enumerate(var_names):
...
def forecast_bvar_multistep(bvar_train: dict, train_y: np.ndarray,
    ...
    for d in range(nd):
        ...
        y_lag  = train_y[te - 1: te - p - 1: -1, :].copy()  # shape (p, N)
        state  = y_lag.flatten()                               # (N*p,)
...
horizons  = [1, 2, 3]
# Map dates to period indices for rolling
...
for fh in horizons:
    ...
    indices = range(start_idx, len(y_data) - fh)
...
for ax_i, fh in enumerate(horizons):
    ...
    ax.set_xlabel("Дата"); ax.set_ylabel("ВВП м/м")
    ax.legend(loc="lower left")
fig.suptitle("BVAR: Rolling OOS прогноз ВВП", fontsize=13, fontweight="bold")
plt.tight_layout()
plt.savefig("bvar_rolling_oos.png", dpi=120)
plt.show()
print("Saved: bvar_rolling_oos.png")

# ================================================================
# ПРОГНОЗ НА 3 И 6 МЕСЯЦЕВ ВПЕРЁД (fan chart)
# ================================================================

print("\n" + "=" * 65)
print("   ПРОГНОЗ ВВП НА 3 И 6 МЕСЯЦЕВ ВПЕРЁД (вся выборка)")
print("=" * 65)
...
future_dates = [last_date + pd.DateOffset(months=i) for i in range(1, max_fh + 1)]

# Print summary
for fh in horizons_future:
    fc    = all_paths[fh - 1, :]
    fdate = future_dates[fh - 1]
    print(f"\n--- Прогноз на {fh} месяцев вперёд (дата: {fdate.strftime('%Y-%m')}) ---")
...
n_hist   = 24
hist_idx = slice(max(0, te_full - n_hist), te_full)
hist_df  = pd.DataFrame({
    "date": dates.iloc[hist_idx].values,
    "gdp":  y_data[hist_idx, gdp_idx],
})
...
ax.plot(fan_df["date"],  fan_df["med"] * 100,  color="blue",  lw=1.5, linestyle="--", label="Медиана прогноза")
...
ax.set_title("BVAR: Прогноз ВВП на 3 и 6 месяцев вперёд", fontsize=13, fontweight="bold")
ax.set_subtitle = ax.set_xlabel
ax.set_xlabel(f"Дата  (последнее наблюдение: {last_date.strftime('%Y-%m')})")
ax.set_ylabel("ВВП м/м, %")
ax.legend(loc="lower left")
ax.set_caption = None
fig.text(0.5, -0.01, "Заливка: 50% / 80% / 90% доверительные интервалы",
         ha="center", fontsize=9, color="gray")
plt.tight_layout()
plt.savefig("bvar_fan_chart.png", dpi=120)
plt.show()
print("Saved: bvar_fan_chart.png")
