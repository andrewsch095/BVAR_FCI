...
def compute_impulse_responses(var_result: VARResult, n_periods: int = 20, shock_size: float = 1.0):
    """Вычисление импульсных откликов с доверительными интервалами"""
    P = var_result.lags
    ...
    if n_draws > 0:
        ...
        return {'median': irf, 'lower': None, 'upper': None}
...
def plot_gdp_impulse_responses(irf_results, var_names, var_idx=0, n_periods=20):
    ...
    for i in range(n_vars):
        ...
        # Основная линия (медиана или точечная оценка)
        ax.plot(periods, response_median, 'b-', linewidth=2.5, 
                label='IRF' if not has_ci else 'Median IRF')
        ...
        y_max = np.max(np.abs(response_median))
        if has_ci:
